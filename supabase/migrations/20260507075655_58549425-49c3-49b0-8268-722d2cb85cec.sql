...
-- Step 5D Two-Key Console: minimal authenticated RPCs.
-- All functions are SECURITY DEFINER and require auth.uid() (no service-role bypass for proof).
-- Scope: governance-internal tables only. No external surface.

-- =====================================================================
-- 1. CREATE REHEARSAL (admin-only)
-- =====================================================================
...
BEGIN
  ...
  INSERT INTO public.vos_proposal_queue (
    ...
    0.99, 'Two-key repeatability proof. Internal only.',
    true, false, true,
...
BEGIN
  ...
  IF v_appr.app_id <> 'vanto_os_console' OR v_appr.approval_type <> 'internal_admin_note_record' THEN
...
-- =====================================================================
-- 3. SECOND REVIEW (governance_reviewer-only, distinct from first)
-- =====================================================================
...
BEGIN
  ...
  -- Mark related rehearsal log row as completed (best-effort, append-only via guard)
  UPDATE public.vos_role_rehearsal_log
   ...
   WHERE rehearsal_kind = 'live_two_key'
...
-- Lock down: revoke from public/anon, grant only to authenticated
REVOKE ALL ON FUNCTION public.vos_step5d_create_rehearsal() FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.vos_step5d_first_review(uuid) FROM PUBLIC, anon;
REVOKE ALL ON FUNCTION public.vos_step5d_second_review(uuid) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.vos_step5d_create_rehearsal() TO authenticated;
GRANT EXECUTE ON FUNCTION public.vos_step5d_first_review(uuid) TO authenticated;
GRANT EXECUTE ON FUNCTION public.vos_step5d_second_review(uuid) TO authenticated;
