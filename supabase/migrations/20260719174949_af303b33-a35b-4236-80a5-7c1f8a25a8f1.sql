...
BEGIN
  ...
  IF TG_OP = 'UPDATE' THEN
    ...
    -- State machine. Solo Mode is allowed to move requested/reviewed directly to second_reviewed.
    IF NEW.approval_status IS DISTINCT FROM OLD.approval_status THEN
      IF NOT (
        ...
        OR (v_solo = 'on' AND OLD.approval_status IN ('requested','reviewed') AND NEW.approval_status = 'second_reviewed')
      ) THEN
        RAISE EXCEPTION 'invalid_status_transition: % -> %', OLD.approval_status, NEW.approval_status;
      END IF;
    END IF;

    -- Two-key on second_reviewed. Solo Mode bypasses the distinct-human check only when the secure RPC sets the flag.
    IF NEW.approval_status = 'second_reviewed' THEN
      ...
      END IF;
      IF v_solo IS DISTINCT FROM 'on' AND NEW.second_reviewed_by = NEW.reviewed_by THEN
