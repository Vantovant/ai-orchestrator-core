...
export default function PortfolioPartnerChat({ projects }: PortfolioPartnerChatProps) {
  ...
  const handleSend = async () => {
    ...
    await portfolioChatService.sendChatSSE(
      ...
      async (fullText, structured) => {
        setStreaming(false);
        setStreamText("");
        const assistantMsg = await portfolioChatService.addMessage(threadId!, "assistant", fullText, [], structured);
        setMessages(prev => [...prev, assistantMsg]);
        // Update thread title from first exchange
        if (messages.length === 0) {
          const titleSlice = prompt.slice(0, 50);
          await portfolioChatService.updateThread(threadId!, { title: titleSlice } as any);
          loadThreads();
        }
      },
      (err) => {
          ...
          <ScrollArea className="flex-1">
            <div className="p-1 space-y-0.5">
              {loadingThreads ? (
                ...
                >
                  {t.is_pinned && <Pin className="h-3 w-3 text-warning shrink-0" />}
                  <MessageSquare className="h-3 w-3 shrink-0 text-muted-foreground" />
                  <span className="truncate flex-1">{t.title}</span>
                  <div className="hidden group-hover:flex gap-0.5">
                    <button onClick={(e) => { e.stopPropagation(); handlePinThread(t.id, t.is_pinned); }} className="p-0.5 hover:text-primary">
                      <Pin className="h-3 w-3" />
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); handleDeleteThread(t.id); }} className="p-0.5 hover:text-destructive">
